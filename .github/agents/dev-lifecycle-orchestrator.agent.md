...
13. 定期実行方式は要求に従って判定する（ローカル cron / GitHub Actions）。要求にない方式を必須化しない。
...
- implementation:
	...
	- 必須: `20_project/22_src/main.py` が存在する
	- 必須: `20_project/22_src/config.json` が存在する
	- 必須: `20_project/22_src/requirements.txt` が存在する
	- 必須: `20_project/22_src/.env.example` が存在する
...
- release:
	...
	- 必須: `README.md` に定期実行方式（ローカル cron または GitHub Actions）が記載されている
	- 条件付き必須: GitHub Actions 運用時のみ `.github/workflows/price_check.yml` が存在する
	- 条件付き必須: ローカル運用時のみ `README.md` に cron 設定例と手動テスト実行手順がある
