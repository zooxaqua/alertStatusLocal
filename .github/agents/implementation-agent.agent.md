...
- 技術要件は `alertstatus-domain` skill の `assets/technical-spec-local-browser.md` を参照し、要求に指定された運用方式（ローカル cron / GitHub Actions）を適用すること。
...
## 成果物
- `20_project/22_src/main.py`（価格取得・通知コアロジック）
- `20_project/22_src/config.json`（監視対象リスト）
- `20_project/22_src/requirements.txt`（依存ライブラリ）
- `20_project/22_src/.env.example`（ローカル環境変数サンプル）
